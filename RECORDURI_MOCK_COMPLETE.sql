-- =============================================
-- RECORDURI MOCK COMPLETE - FISH TROPHY
-- Cu detalii complete și realiste
-- =============================================

-- Verifică dacă există utilizatori (dacă nu, creează unul mock)
DO $$
DECLARE
  user_count INTEGER;
  mock_user_id UUID;
BEGIN
  ...
  IF user_count = 0 THEN
    ...
    -- Inserează în auth.users (simulat)
  ...
  ELSE
    -- Folosește primul utilizator existent
    SELECT id INTO mock_user_id FROM public.profiles LIMIT 1;
    RAISE NOTICE '✅ Folosind utilizatorul existent';
  END IF;
END $$;

-- Șterge recordurile existente (dacă există)
DELETE FROM public.records;

-- Adaugă recorduri mock cu detalii complete
...
BEGIN
  ...
  RAISE NOTICE '✅ Recorduri mock adăugate cu succes!';
  RAISE NOTICE '📊 Total recorduri: %', record_count;
  RAISE NOTICE '🎣 Specii incluse: Crap, Somn, Știucă, Biban, Platca, Caras, Lin, Avat, Clean, Mreană, Zander, Biban mare, Crap argintiu, Crap auriu, Somn alb';
  RAISE NOTICE '📍 Locații: Lacul Băneasa, Dunărea, Lacul Snagov, Lacul Herăstrău, Lacul Cernica, Lacul Morii, Lacul Tei, Lacul Floreasca, Lacul Colentina';
