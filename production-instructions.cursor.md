# Romanian Fishing Hub – Production Instructions (Cursor-Ready)
...
- [ ] Node.js **>= 20.x**, pnpm (or npm), Git
- [ ] ESLint + Prettier + Husky + lint-staged
- [ ] EditorConfig, TypeScript strict, path aliases (`@/*`)
- [ ] Vercel project linked to GitHub (Preview deployments)

#### 1.2 Database (Neon + PostGIS) & ORM
- [ ] Create **Neon** project → get `DATABASE_URL`
- [ ] Enable extensions in Neon (SQL):
  ```sql
  CREATE EXTENSION IF NOT EXISTS postgis;
  CREATE EXTENSION IF NOT EXISTS postgis_topology;
  CREATE EXTENSION IF NOT EXISTS pgcrypto; -- for gen_random_uuid if needed
  ```
- [ ] Add **Drizzle** (`drizzle-kit`) config and migrations path
- [ ] Create schema (see “Database Design” section) and run:
  ```bash
...
#### 1.3 Authentication
- [ ] Create Firebase project + Web app, enable **Email/Password** + **Google**
- [ ] Frontend: initialize Firebase SDK (`VITE_FIREBASE_*`)
- [ ] Server: setup Firebase Admin in Vercel Functions (read envs, verify ID token)
...
- [ ] Firebase Auth flows (register/login with Email & Google)
- [ ] Store minimal profile in DB (`users`) and set **custom role claim** (`user|moderator|admin`)
- [ ] Middleware in Vercel Functions: verify `Authorization: Bearer <id_token>`
...
#### 3.1 Auth Components
- [ ] Auth modal (login/register with Email/Google) via Firebase
- [ ] `useAuth()` hook (states: loading/null/user) + ProtectedRoute
- [ ] Toasts & error boundaries

#### 3.2 Interactive Map (Leaflet)
- [ ] Base map with OSM tiles + attribution
- [ ] **Water bodies** (polygons) layer (GeoJSON)
- [ ] **Locations** (points), **Amenities** (points) with toggles
- [ ] **Popups** on water body:
  - top records summary
  - main species list
  - “Adaugă record” button (opens form)
- [ ] BBox queries: fetch only what’s in view (server accepts `bbox`)

#### 3.3 Navigation & Theming
- [ ] Responsive navbar, footer, layout
- [ ] Theme via CSS variables (Tailwind) — default + **Black Sea** palette
- [ ] Route: `/black-sea` with light‑blue accent + preset filters
...
#### 4.3 User Profiles
- [ ] Profile page with stats (total records, PBs, last catches)
- [ ] Public profile link
- [ ] Preferences: email notifications (approved/rejected, record beaten)
