...
export default function Home() {
  ...
  // Funcție pentru deschiderea popup-ului de shop
  const openShopPopup = () => {
  ...
  // Funcție pentru schimbarea stilului hărții
  const changeMapStyle = (newStyle: 'osm' | 'satellite' | 'hybrid') => {
    if (!mapInstanceRef.current) return;
    
    setMapStyle(newStyle);
    const map = mapInstanceRef.current;

    // Așteaptă până când stilul se încarcă
    map.once('style.load', () => {
      // Re-adăugă markerii după ce stilul s-a încărcat
      setTimeout(() => {
        if (map.isStyleLoaded()) {
          // Re-adăugă toate markerii
