...
export default function Home() {
  ...
  const ajvpsMarkersRef = useRef<AJVPSOfficeMarker[]>([]);
  const accommodationMarkersRef = useRef<AccommodationMarker[]>([]);
  ...
  const addLocationsToMap = (_map: maplibregl.Map, filterType: string) => {
    ...
    try {
      ...
      if (filterType === 'all') {
        filtered = sourceData;
      } else if (filterType === 'fishing_location') {
        // Show all fishing locations (river, lake, etc.)
        filtered = sourceData;
      } else if (filterType === 'river') {
        filtered = sourceData.filter(loc => loc.type === 'river' || loc.type === 'fluviu');
      } else if (filterType === 'shop' || filterType === 'ajvps_office' || filterType === 'accommodation') {
        // These types are handled by addAllLocationTypesToMap
        filtered = [];
      } else {
        filtered = sourceData.filter(loc => loc.type === filterType);
      }
  ...
  // Helper function to add all location types to map
  const addAllLocationTypesToMap = (_map: maplibregl.Map, filterType: string) => {
    if (!_map || !_map.getContainer()) return;

    // First, add fishing locations using existing logic
    addLocationsToMap(_map, filterType);
    ...
    const addLayerForType = (
      ...
      generateCardFn: (data: any, isMobile: boolean) => string
    ) => {
      ...
      const clickHandlerKey = `${layerId}-click-handler`;
      if (!(_map as any)[clickHandlerKey]) {
        _map.on('click', layerId, async (e) => {
          if (!e.features || !e.features[0]) return;
          const coordinates = (e.features[0].geometry as any).coordinates.slice();
          const properties = e.features[0].properties;
          ...
          // Calculate offset
          const containerHeight = _map.getContainer().clientHeight;
          const offsetY = containerHeight * 0.10;
          const targetZoom = 14;
          const pixelsAtTargetZoom = 256 * Math.pow(2, targetZoom);
          const degreesPerPixel = 360 / pixelsAtTargetZoom;
          const offsetLat = offsetY * degreesPerPixel;
          ...
          const currentZoom = _map.getZoom();
          const markerRadius = currentZoom <= 5 ? 10 : currentZoom <= 10 ? 14 : 18;
          const popupOffset = -(markerRadius + 5);
          ...
          // Load details and show card
          const details = await getDetailsFn(properties.id);
          if (details) {
            details.coords = coordinates;
            const cardHTML = generateCardFn(details, isMobile);
            loadingPopup.setHTML(cardHTML);
          } else {
            loadingPopup.setHTML('<div class="p-4 text-red-500">Eroare la încărcare</div>');
          }
        });
        ...
        (_map as any)[clickHandlerKey] = true;
      }
    };

    // Show/hide fishing locations layer based on filter
    if (filterType !== 'all' && !['fishing_location', 'river', 'lake', 'balti_salbatic', 'private_pond'].includes(filterType)) {
      if (_map.getLayer('location-circles')) {
        _map.setLayoutProperty('location-circles', 'visibility', 'none');
      }
    } else {
      if (_map.getLayer('location-circles')) {
        _map.setLayoutProperty('location-circles', 'visibility', 'visible');
      }
    }
    ...
    if (filterType === 'all' || filterType === 'shop') {
      if (shopMarkers.length > 0) {
        addLayerForType(
          ...
          'shop',
          async (id) => {
            const { data } = await supabase.from('fishing_shops').select('*').eq('id', id).single();
            return data ? { ...data, coords: [data.longitude, data.latitude] } : null;
          },
          generateShopCard
        );
        if (_map.getLayer('shop-circles')) {
          _map.setLayoutProperty('shop-circles', 'visibility', 'visible');
        }
      }
    } else {
      // Hide shops layer
      if (_map.getLayer('shop-circles')) {
        _map.setLayoutProperty('shop-circles', 'visibility', 'none');
      }
    }

    // Add AJVPS offices
    if (filterType === 'all' || filterType === 'ajvps_office') {
      if (ajvpsMarkers.length > 0) {
        addLayerForType(
          'ajvps-offices',
          'ajvps-circles',
          ajvpsMarkers,
          '#ec4899',
          'ajvps_office',
          getAJVPSOfficeDetails,
          generateAJVPSOfficeCard
        );
        if (_map.getLayer('ajvps-circles')) {
          _map.setLayoutProperty('ajvps-circles', 'visibility', 'visible');
        }
      }
    } else {
      if (_map.getLayer('ajvps-circles')) {
        _map.setLayoutProperty('ajvps-circles', 'visibility', 'none');
      }
    }

    // Add accommodations
    if (filterType === 'all' || filterType === 'accommodation') {
      if (accommodationMarkers.length > 0) {
        addLayerForType(
          'accommodations',
          'accommodation-circles',
        ...
        if (_map.getLayer('accommodation-circles')) {
          _map.setLayoutProperty('accommodation-circles', 'visibility', 'visible');
        }
      }
    } else {
      if (_map.getLayer('accommodation-circles')) {
        _map.setLayoutProperty('accommodation-circles', 'visibility', 'none');
      }
    }
  };
    ...
    filterDebounceRef.current = setTimeout(() => {
      ...
      if (mapInstanceRef.current && databaseLocations.length > 0) {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            if (!isAddingMarkers) {
              addAllLocationTypesToMap(map, type);
            }
          });
        });
      }
  ...
  const changeMapStyle = (style: 'osm' | 'satellite' | 'hybrid') => {
    ...
    // Change visibility only (like MapEditor) - markers persist
    if (style === 'satellite') {
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1.5 md:gap-2 justify-center">
                  {[
                    ...
                    { type: 'fishing_location', label: 'Locații Pescuit', icon: MapPin, color: 'bg-blue-500 hover:bg-blue-600' },
                    ...
                    const count = type === 'all'
                      ? databaseLocations.length + shopMarkers.length + ajvpsMarkers.length + accommodationMarkers.length
              ...
              <div className="border-t border-gray-200 pt-3">
                ...
                <div className="flex flex-wrap gap-1.5 md:gap-2 justify-center">
                  {[
                    ...
                    { type: 'ajvps_office', label: 'Birouri AJVPS', icon: MapPin, color: 'bg-pink-500 hover:bg-pink-600', count: ajvpsMarkers.length },
                    { type: 'accommodation', label: 'Cazări', icon: MapPin, color: 'bg-orange-500 hover:bg-orange-600', count: accommodationMarkers.length }
                  ].map(({ type, label, icon: Icon, color, count }) => (
                    <button
                      key={type}
                      onClick={() => filterLocations(type)}
                      className={`${color} text-white px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-medium transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-1.5 hover:scale-105 ${activeFilter === type ? 'ring-2 ring-blue-200' : ''
