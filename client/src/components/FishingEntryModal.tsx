...
const FishingEntryModal: React.FC<FishingEntryModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    video_url: '',
    youtube_url: '', // New separate field
  ...
  const photosRequired = isRecord; // Required for all records
  const videoRequired = isRecord; // Required for all records
    ...
    if (entry && isOpen && isEdit) {
      ...
      const loadEntryId = async () => {
        ...
        setFormData(prev => ({
          ...
          youtube_url: (entry as any).youtube_url || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isLikelyYoutube = (url: string) => url.includes('youtube.com') || url.includes('youtu.be');
    const isExistingVideoYoutube = entry?.video_url && isLikelyYoutube(entry.video_url);

    // Validation:
    // 1. Photo required (checked above)
    // 2. Video required:
    //    - For upload: need file OR existing video_url (that is NOT youtube link, assuming we migrated)
    //    - But user requirements say: "Video upload logic... youtube separate"
    ...
    if (!isAdmin) {
      ...
      // Video logic: Mandatory upload
      // If add mode: Must have video_file
      // If edit mode: Must have video_file OR (existing video_url AND NOT youtube)

      const hasFile = !!formData.video_file;
      const hasExistingVideo = !!entry?.video_url && !isLikelyYoutube(entry.video_url);

      if (!hasFile && (!isEdit || !hasExistingVideo)) {
        toast.error('Videoclipul este obligatoriu (încărcare fișier)');
        return;
      }

      // YouTube optional validation
      if (formData.youtube_url && !extractYouTubeId(formData.youtube_url)) {
